...
public class ZoomageView extends AppCompatImageView implements OnScaleGestureListener {
    ...
    private void animateTranslationX() {
        if (getCurrentDisplayedWidth() > getWidth()) {
            ...
            if (bounds.left > 0) {
                if(bounds.left > 0&&bounds.left <= getWidth()/2){
                    animateMatrixIndex(Matrix.MTRANS_X, bounds.left );
                }else {
                    animateMatrixIndex(Matrix.MTRANS_X, getWidth()/2);
                }
            ...
            //the right edge is too far to the interior
                ...
                if(bounds.right > 0&&bounds.right <= getWidth()/2){
                    animateMatrixIndex(Matrix.MTRANS_X, bounds.left + getWidth()/2 - bounds.right);
                }else {
                    animateMatrixIndex(Matrix.MTRANS_X, bounds.left );
                }
...
//                animateMatrixIndex(Matrix.MTRANS_X, bounds.left + getWidth() - bounds.right);
    ...
    private void animateTranslationY() {
        if (getCurrentDisplayedHeight() > getHeight()) {
            ...
            if (bounds.top > 0) {
                if(bounds.top > 0&&bounds.top <= getHeight()/2){
                    animateMatrixIndex(Matrix.MTRANS_Y, bounds.top );
                }else {
                    animateMatrixIndex(Matrix.MTRANS_Y, getHeight()/2);
                }
...
//                animateMatrixIndex(Matrix.MTRANS_Y, 0);
            ...
            //the bottom edge is too far to the interior
                ...
                if(bounds.bottom > 0&&bounds.bottom <= getHeight()/2){
                    animateMatrixIndex(Matrix.MTRANS_Y, bounds.top + getHeight()/2 - bounds.bottom);
                }else {
                    animateMatrixIndex(Matrix.MTRANS_Y, bounds.top);
                }
